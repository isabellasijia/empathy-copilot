...
> 这份文档回答三个问题：项目有哪些页面、客户一句话如何走完整条链路、后端工作流实际执行了哪些节点。
...
| 效果评测台 | `/evaluation` | 可复算回归场景；理解/安全指标；真实执行节点；RAG 召回分数；最近一次 Qwen 耗时与 token |
...
系统不声称存在多个自治 Agent。`WorkflowTrace` 仅在代码真正执行一个节点后追加记录，未执行的节点不会出现在轨迹中。

| 可能出现的节点 | 实现方式 | 触发条件 / 用途 |
| --- | --- | --- |
| 读取服务上下文 | SQLite + 本地 Python | 每次分析实际读取聊天、订单、工单和记忆 |
| 本地规则分析 | 确定性规则 | 每次新分析生成意图候选、情绪、风险和进度 |
| 歧义意图复核 | Qwen 文本模型 | 本地候选接近、超范围或低置信时 |
| 图片核对 | Qwen Omni 多模态 | 有真实可读取图片、已配置 Key 且进入分析模型分支时 |
| 知识检索 | 本地 RAG | 生成复杂业务草稿时 |
| 发送前检查 | 确定性规则 + 按需 Qwen | 客服发送最终文本时 |

每条轨迹都包含实际耗时、执行引擎、触发原因、输入输出摘要和状态迁移。模型节点额外记录模型名称与 Token；需要人工时记录中断原因。
...
- **可解释性**：返回 evidence、provider、model、token、latency、实际执行节点和状态迁移。
...
flowchart LR
    ...
    I --> J[消费者端自动收到回复]
    J --> K[进入风险台查看证据和负责人]
    K --> L[打开评测台展示实际执行轨迹、可复算结果与 RAG]
```

推荐双端地址：

- 客服工作台：`http://127.0.0.1:4173/?case=S00018#assist`
...
| 实际工作流轨迹 | `backend/app/orchestration.py` |
